...
// Define send via and auto-delete options
const sendViaOptions = ['EMAIL', 'SMS', 'PUSH']
const autoDeleteOptions = ['7 days', '30 days', 'Never']
...
export default function ReminderForm() {
  ...
  const { reminderTab, formData, loading } = useSelector(
    (state: RootState) => state.reminder,
  )
  const { serviceOptions, isLoading: servicesLoading } = useSelector(
    (state: RootState) => state.service,
  ) // Access serviceOptions
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isPreviewOpen, setIsPreviewOpen] = useState(true)
  ...
  useEffect(() => {
    if (!serviceOptions.length) {
      dispatch(fetchServices(false)) // Fetch services without manual refresh
    }
  }, [dispatch])
  ...
  // Map form data to API format and submit
  const onSubmit = async (data: FormData) => {
    setIsSubmitting(true)
    try {
      // Map form data to API format based on Prisma schema
      const apiData = {
        type: data.type.toUpperCase().replace('-', '_'), // e.g., 'Upcoming' -> 'REMINDER', 'Follow-up' -> 'FOLLOW_UP'
        title: data.subject,
        description: data.description,
        message: data.message,
        services: data.service ? [data.service] : [], // Send service ID as an array
        notifications: data.notifications.map((method) => ({ method })),
        reminderOffset: data.customSchedules.length
          ? data.customSchedules.map((schedule) => ({
              customScheduleAt: new Date(
                `${schedule.date}T${schedule.time}`,
              ).toISOString(),
              sendBefore: true,
              sendOffset: null,
            }))
          : data.when.map((offset) => {
              // Convert human-readable offsets to minutes
              const offsetMap: Record<string, number> = {
                '1 hours before appointment': 60,
                '24 hours before appointment': 1440,
                '48 hours before appointment': 2880,
                '1 hour after appointment': -60,
              ...
              return {
                customScheduleAt: null,
                sendBefore: !offset.includes('after'),
                sendOffset: offsetMap[offset] || null,
              }
            }),
      }
      console.log('Submitting API data:', apiData) // Log API data for debugging
      await dispatch(createReminder(apiData)).unwrap()
      dispatch(resetFormData())
      reset(formData)
    } catch (error) {
      // Error is handled by the thunk via toast
    } finally {
      setIsSubmitting(false)
    }
  }
        ...
        <div className="grid grid-cols-12 relative gap-4 h-full">
          ...
          <div className="col-span-12 lg:col-span-9 space-y-6">
            ...
            <Tabs
              value={reminderTab}
              onValueChange={(value) =>
                dispatch(setReminderTab(value as ReminderSubTab))
              }
            >
              <TabsList className="grid gap-1 grid-cols-5">
                {Object.values(ReminderSubTab).map((tab) => (
            ...
            <ReminderSelectField
              name="service"
              label="Choose Service"
              options={serviceOptions} // Use serviceOptions from serviceSlice
              placeholder={
                servicesLoading
                  ? 'Loading services...'
                  : 'Select service to set reminder'
              }
              icon={SlidersHorizontal}
              disabled={servicesLoading} // Disable while loading
            />
            ...
            <Button
              type="submit"
              className="w-full"
              disabled={isSubmitting || loading || servicesLoading}
            >
              {isSubmitting || loading ? 'Submitting...' : 'Create Reminder'}
            </Button>
          </div>
          {/* Live Preview */}
          <div className="col-span-12 h-fit sticky top-0 lg:col-span-3 bg-white shadow border rounded-md p-4 overflow-y-auto">
